# Policy Small - Supervised Learning from MCTS

Train a small policy network to match MCTS visit distributions from collected game logs.
...
```bash
policy-small-train --mcts-logs-dir scoundrel/rl/mcts/logs/collected_games --epochs 100
```
...
```bash
policy-small-viewer --checkpoint checkpoints/policy_small_epoch_100.pt
```
...
- Single MLP for scalar features (current room & status + dungeon stack sums)
- Vector of scalars for unknown health (potions), weapon, and monsters (sums of dungeon stack)
