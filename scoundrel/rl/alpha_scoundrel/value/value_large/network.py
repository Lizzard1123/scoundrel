...
class ValueLargeNet(nn.Module):
    """
    Value network architecture identical to PolicyLargeNet but with value head instead of action head.
    
    Architecture:
    - Transformer encoder for sequence data (dungeon stack)
    - MLP for scalar features (current room & status)
    - Single value head (outputs scalar expected value)
    """
    
    def __init__(self, scalar_input_dim):
        super(ValueLargeNet, self).__init__()
    ...
    def forward(self, scalar_data, sequence_data):
        ...
        Returns:
            value: [batch_size, 1] tensor of expected value
        """
